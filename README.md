...
Для работы машины нужно задается программа и её начальное состояние (то есть состояние ленты и позицию каретки). 
...
3. !
```
Если какая-то строка программы содержит ошибку, выводится сообщение `Error` в выходной файл (имя файла задается третьим аргументом командной строки).
...
```
задает на ленте, замкнутой в кольцо, на которой 1000 ячеек исходную строку `111011111`, и максимальное количество тактов работы машины 2000000.

Входная информация на ленте помещается, начиная с нулевой ячейки. Начальная позиция каретки над нулевой ячейкой. 

Если в ходе работы МП не остановилась за максимально разрешенное количество тактов работы, машина останавливается и выводится в выходной файл (имя файла задается третим аргументом командной строки) сообщение `Not applicable`.
В остальных случаях выводится в файл содержимое ленты, начиная с позиции, на которой остановилась МП, и до последней 1 на ленте.
